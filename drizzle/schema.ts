...
export const books = pgTable(
  "books",
  {
    ...
    pdfFileKey: varchar("pdfFileKey", { length: 255 }).default('').notNull(),
    pdfFileUrl: varchar("pdfFileUrl", { length: 1024 }).default('').notNull(),
    pageCount: integer("pageCount").default(0).notNull(),
    processingStatus: bookStatusEnum("processingStatus").default("pending").notNull(),
    totalPrice: numeric("totalPrice", { precision: 10, scale: 2 }).default('0.00').notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().notNull(),
  },
  (table) => [
